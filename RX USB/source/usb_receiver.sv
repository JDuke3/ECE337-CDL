...
RX_edge_det edg (.clk(clk), .n_rst(n_rst), .DP_sync(DP_sync), .edge_flag(edge_flag));
RX_NRZI_Decoder mod3(.clk(clk), .n_rst(n_rst), .DP_sync(DP_sync), .DM_sync(DM_sync), 
.shift_strobe(shift_strobe), .EOP(EOP), .d_orig(d_orig), .d_enable(d_enable));
RX_timer tim (.clk(clk), .n_rst(n_rst), .clear(RX_Data_ready), .count_enable(d_enable), 
.rollover_val(5'd25), .rollover_flag(shift_strobe), .count_out());
...
RX_flex_sr #(16, 1) shift_left (.clk(clk), .n_rst(n_rst), .shift_enable(shift_strobe), 
...
.Pid_err(Pid_err), .EOP(EOP), .byte_done(byte_done), .crc_err(crc_err), .d_enable(d_enable), .trans_type(trans_type), 
.parallel_out(parallel_out), .Flush(Flush), .Store_RX_Packet_Data(Store_RX_Packet_Data), .RX_Error(RX_Error), .RX_Data_ready(RX_Data_ready), .RX_Transfer_Active(RX_Transfer_Active));
